...
import { UPDATE_DEBT_CUSTOMER } from "../../admin/api";
import Loading from "../loading";
import { http } from "../../admin/utils/http";

export default function AlertDialogCustomer({
  handleCloseModal,
  isModalOpen,
  isLoadings,
  customerId,
  ...
  // Sửa dữ liệu
  const updateDebt = async (formData) => {
    try {
      const response = await http.put(
        UPDATE_DEBT_CUSTOMER + customerId,
        formData
      );
      setLoading(false);
  ...
  const mutation = useMutation(updateDebt, {
    onSuccess: () => {
        ...
        >
          <div className="mx-4 mt-4">
            <h2 className="text-xl font-medium">
              Thêm công nợ: {item_customer?.fullname}
            </h2>
          </div>
          <DialogActions>
