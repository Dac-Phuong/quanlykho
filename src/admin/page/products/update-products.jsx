      ...
      await http
        ...
        .then((response) => {
          if (response.status === 200) {
            ...
            setValue(response.data.product_group.find((item) => item.id === response?.data?.item?.group) || "");
            const name = response.data.product_group.find((item) => item.id === response?.data?.item?.group).group_name;
            setGroup_name(name);
            setFormData({
