...
import { CREATE_LOCATION } from '../../api'
...
export default function CreateLcation() {
    const Title = 'Quản lý tuyến'
    const [loading, setIsLoading] = useState(false)
    const queryClient = useQueryClient()
    const queryKey = 'location_key'
    ...
    const createLocation = async (formData) => {
        try {
            const response = await http.post(CREATE_LOCATION, formData)
            setIsLoading(false)
            setFormData({
                name: '',
                desc: ''
            })
            showToastSuccess('Thêm tuyến thành công!')
            return response.data
        } catch (error) {
            setIsLoading(false)
            showToastError('Thêm tuyến thất bại!')
            console.log(error)
        }
    }
                ...
                return (
                    <div>
                        <Link
                            ...
                            to={`/khach-hang/quan-ly-tuyen/sua-tuyen/${params?.row?.id}`}
                        >
                            sửa
                        </Link>
                    </div>
    ...
    const rows =
        data?.data?.map((item, index) => ({
            ...
            name: item.name,
            desc: item.desc,
            created_at: item.created_at != null ? dayjs(item.created_at).format('YYYY-MM-DD') : null
        })) || []
        ...
        <div className='pcoded-content'>
            ...
            <div className=' m-4'>
                <div className='card'>
                    ...
                    <div className='card-block remove-label'>
                        <DataGridCustom rows={rows} columns={columns} nameItem={'quản lý tuyến'} />
                    </div>
                </div>
            </div>
