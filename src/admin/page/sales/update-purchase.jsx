...
import { TextField, Autocomplete, MenuItem, TextareaAutosize, Select } from '@mui/material/'
                ...
                await http
                    ...
                    .then((response) => {
                        if (response.status === 200) {
                            setLoading(false)
                            setStaff(data?.staff.find((ite) => ite.id == response?.data?.Sales?.staff_id) || null)
                            setCustomer(
                                data?.customers.find((item) => item.id == response?.data?.Sales?.customer_id) || null
                            )
